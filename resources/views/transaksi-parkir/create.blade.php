...
@section('title', 'Tambah Transaksi - PinkPark')
@section('header', 'Tambah Transaksi Parkir')

@section('content')
<div class="card shadow-sm border-0 rounded-4">
    <div class="card-header bg-white border-0 pt-4 pb-3 px-4">
        <h5 class="fw-bold mb-0 text-dark">Form Tambah Transaksi</h5>
    </div>
        ...
        <form action="{{ route('transaksi.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('kode_transaksi') is-invalid @enderror" id="kode_transaksi" name="kode_transaksi" value="{{ old('kode_transaksi', 'TRX-' . time()) }}" required readonly>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('member_id') is-invalid @enderror" id="member_id" name="member_id" required>
                        ...
                        @foreach($members as $member)
                            <option value="{{ $member->id }}" data-nopol="{{ $member->nomor_polisi }}" data-jenis="{{ $member->jenis_kendaraan }}" {{ old('member_id') == $member->id ? 'selected' : '' }}>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('nomor_polisi') is-invalid @enderror" id="nomor_polisi" name="nomor_polisi" value="{{ old('nomor_polisi') }}" placeholder="Otomatis terisi" required readonly>
                    @error('nomor_polisi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('jenis_kendaraan') is-invalid @enderror" id="jenis_kendaraan" name="jenis_kendaraan" value="{{ old('jenis_kendaraan') }}" placeholder="Otomatis terisi" required readonly>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="datetime-local" class="form-control @error('waktu_masuk') is-invalid @enderror" id="waktu_masuk" name="waktu_masuk" value="{{ old('waktu_masuk', now()->format('Y-m-d\TH:i')) }}" required>
                    @error('waktu_masuk')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label for="tarif" class="form-label fw-medium">Tarif Dasar (Rp) <span class="text-danger">*</span></label>
                    <input type="number" class="form-control @error('tarif') is-invalid @enderror" id="tarif" name="tarif" value="{{ old('tarif', 0) }}" required min="0">
                    @error('tarif')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

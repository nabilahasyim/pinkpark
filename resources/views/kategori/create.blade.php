...
@section('title', 'Tambah Kategori - PinkPark')
@section('header', 'Tambah Kategori')

@section('content')
<div class="card shadow-sm border-0 rounded-4">
    <div class="card-header bg-white border-0 pt-4 pb-3 px-4">
        <h5 class="fw-bold mb-0 text-dark">Form Tambah Kategori</h5>
    </div>
    ...
    <div class="card-body px-4 pb-4">
        <form action="{{ route('kategori.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('kode_kategori') is-invalid @enderror" id="kode_kategori" name="kode_kategori" value="{{ old('kode_kategori') }}" placeholder="Contoh: KTG-001" required>
                @error('kode_kategori')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nama_kategori') is-invalid @enderror" id="nama_kategori" name="nama_kategori" value="{{ old('nama_kategori') }}" placeholder="Contoh: Kendaraan Roda Dua" required>
                @error('nama_kategori')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
